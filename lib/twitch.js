...
const TWITCH_CHATBOT_ACCESS_TOKEN_FN = `${__dirname}/../data/twitchChatBotToken.json`;
...
async function createTwitchUserClient(context, chatbot = false) {
  const { config, log } = context;
  const { clientId, clientSecret } = config.twitch.api;
  const tokenData = await loadTwitchUserCredentials(context, chatbot);
...
async function loadTwitchUserCredentials(context, chatbot = false) {
  return JSON.parse(
    fs.readFileSync(
      chatbot ? TWITCH_CHATBOT_ACCESS_TOKEN_FN : TWITCH_USER_ACCESS_TOKEN_FN
    )
  );
}

async function updateTwitchUserCredentials(
  context,
  tokenData,
  chatbot = false
) {
